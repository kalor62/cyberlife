...
const server = new Server({ name: 'cyberlife-gmail-mcp', version: '1.0.0' }, { capabilities: { tools: { listChanged: true } } });
// Cache clients per account
const clients = new Map();
async function getClient(account = 'default') {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'gmail_inbox': {
                const { account = 'default', limit = 10 } = args;
                if (!(await isAuthenticated(account))) {
                    return {
                        content: [{
                                type: 'text',
            ...
            case 'gmail_search': {
                const { account = 'default', query, limit = 10 } = args;
                if (!(await isAuthenticated(account))) {
                    return {
                        content: [{
                                type: 'text',
            ...
            case 'gmail_send': {
                const { account = 'default', to, subject, body, cc, bcc } = args;
                if (!(await isAuthenticated(account))) {
                    return {
                        content: [{
                                type: 'text',
            ...
            case 'gmail_reply': {
                const { account = 'default', id, body } = args;
                if (!(await isAuthenticated(account))) {
                    return {
                        content: [{
                                type: 'text',
            ...
            case 'gmail_list_filters': {
                ...
                const formatted = filters.map((f, i) => {
                    const criteria = [];
                    if (f.criteria.from)
                        criteria.push(`from:${f.criteria.from}`);
                    if (f.criteria.to)
                        criteria.push(`to:${f.criteria.to}`);
                    if (f.criteria.subject)
                        criteria.push(`subject:${f.criteria.subject}`);
                    if (f.criteria.query)
                        criteria.push(`query:${f.criteria.query}`);
                    if (f.criteria.hasAttachment)
                        criteria.push('has:attachment');
                    const actions = [];
                    if (f.actions.addLabelIds?.length)
                        actions.push(`+labels: ${f.actions.addLabelIds.join(', ')}`);
                    if (f.actions.removeLabelIds?.length)
                        actions.push(`-labels: ${f.actions.removeLabelIds.join(', ')}`);
                    if (f.actions.forward)
                        actions.push(`forward: ${f.actions.forward}`);
            ...
            case 'gmail_create_filter': {
                const { account = 'default', ...filterOptions } = args;
                if (!(await isAuthenticated(account))) {
                    return {
                        content: [{
                                type: 'text',
            ...
            case 'gmail_create_label': {
                const { account = 'default', name: labelName } = args;
                if (!(await isAuthenticated(account))) {
                    return {
                        content: [{
                                type: 'text',
